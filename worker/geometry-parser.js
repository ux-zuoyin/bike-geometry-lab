...
For each raw row, preserve the source label text as printed and provide one value per detected size in the exact source column order. Use null only for an unreadable cell; never delete an entire row because its field meaning is uncertain. unit and explicitUnit are optional diagnostics only. When present, they must never be inferred from numeric magnitude or bicycle knowledge; downstream code independently verifies row-level units from the visible source label.
