...
- Preview motion uses an 80 RPM cadence baseline: chainring and both cranks complete one linear cycle every 0.75 seconds. Front/rear wheels and rotors run slightly faster at 100 RPM / 0.6 seconds per cycle. The non-drive crank starts from the geometry-mapped point exactly opposite the drive pedal, then receives the same rotation cycle so the 180° phase relationship is preserved.
